...
import { Target, Plus, Edit2, Trash2, PiggyBank, TrendingUp, Calendar, ArrowUpCircle, ArrowDownCircle } from 'lucide-react';
...
export default function SavingsGoals() {
  ...
  function openCreate() {
    setEditing(null);
    setName(''); setTargetAmount(''); setDeadline(''); setColor('#3b82f6'); setFormError('');
    setModalOpen(true);
  }
  ...
  function openEdit(goal) {
    ...
    setDeadline(goal.deadline || ''); setColor(goal.color); setFormError('');
    setModalOpen(true);
  }
  ...
  async function handleSave(e) {
    ...
    try {
      const payload = { name: name.trim(), target_amount: Number(targetAmount), deadline: deadline || null, color };
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={openCreate}
          className="flex items-center gap-1.5 sm:gap-2 bg-blue-600 text-white px-3.5 sm:px-4 py-2 sm:py-2.5 rounded-xl hover:bg-blue-700 transition-colors font-medium text-sm shadow-sm"
        >
          <Plus size={18} />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-4 sm:p-5 text-white">
            <div className="flex items-center gap-2 opacity-80 text-xs sm:text-sm mb-1">
              <PiggyBank size={14} className="sm:hidden" />
              <PiggyBank size={16} className="hidden sm:block" />
              Total Terkumpul
            </div>
            <p className="text-lg sm:text-xl font-bold">
              Rp <span className="sm:hidden">{formatCompact(totalSaved)}</span><span className="hidden sm:inline">{formatRupiah(totalSaved)}</span>
            </p>
            ...
            <p className="text-[10px] sm:text-xs mt-1 opacity-70">
              dari Rp <span className="sm:hidden">{formatCompact(totalTarget)}</span><span className="hidden sm:inline">{formatRupiah(totalTarget)}</span>
            </p>
          </div>
                ...
                <div className="p-4 sm:p-5">
                  <div className="flex items-start justify-between mb-3 sm:mb-4">
                    ...
                    <div className="flex items-center gap-0.5 sm:gap-1 ml-2">
                      <button onClick={() => openFund(goal, 'deposit')} className="p-1.5 sm:p-2 text-green-500 hover:bg-green-50 rounded-lg transition-colors" title="Tambah">
                        <ArrowUpCircle size={18} />
                      </button>
                      <button onClick={() => openFund(goal, 'withdraw')} className="p-1.5 sm:p-2 text-amber-500 hover:bg-amber-50 rounded-lg transition-colors" title="Tarik">
                        <ArrowDownCircle size={18} />
                      </button>
                      <button onClick={() => openEdit(goal)} className="p-1 sm:p-1.5 text-gray-400 hover:bg-gray-50 rounded-lg transition-colors">
                        <Edit2 size={15} />
                      </button>
                      <button onClick={() => handleDelete(goal.id)} className="p-1 sm:p-1.5 text-red-400 hover:bg-red-50 rounded-lg transition-colors">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Target' : 'Target Tabungan Baru'}>
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Contoh: Dana Darurat, Liburan..." className="w-full border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm" autoFocus />
          </div>
          <div>
            ...
            <div className="bg-blue-50 rounded-xl p-3.5 text-sm">
              <p className="text-blue-700">
                ...
                <span className="font-bold">
                  Rp {formatRupiah(fundMode === 'deposit' ? fundGoal.current_amount + Number(fundAmount) : Math.max(fundGoal.current_amount - Number(fundAmount), 0))}
                </span>
