    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 dark:text-gray-500 text-xs sm:text-sm mt-0.5 sm:mt-1 hidden sm:block">Pantau progres tabunganmu menuju impian</p>
        </div>
      ...
      {goals.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          ...
          <div className="hidden sm:block bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm transition-colors">
            <div className="flex items-center gap-2 text-gray-500 dark:text-gray-400 text-sm mb-1"><Target size={16} />Target Aktif</div>
            <p className="text-2xl font-bold text-gray-800 dark:text-gray-100">{goals.length}</p>
          </div>
          <div className="hidden sm:block bg-white dark:bg-gray-800 rounded-2xl p-5 border border-gray-100 dark:border-gray-700 shadow-sm transition-colors">
          ...
          <div className="sm:hidden flex gap-3">
            <div className="flex-1 bg-white dark:bg-gray-800 rounded-xl p-3 border border-gray-100 dark:border-gray-700 text-center transition-colors">
              <p className="text-lg font-bold text-gray-800 dark:text-gray-100">{goals.length}</p>
              <p className="text-[10px] text-gray-400 dark:text-gray-500">Target Aktif</p>
            </div>
            <div className="flex-1 bg-white dark:bg-gray-800 rounded-xl p-3 border border-gray-100 dark:border-gray-700 text-center transition-colors">
              <p className="text-lg font-bold text-green-600 dark:text-green-400">{completedCount}</p>
              <p className="text-[10px] text-gray-400 dark:text-gray-500">Tercapai</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4 sm:p-6 shadow-sm transition-colors">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm sm:text-lg font-semibold text-gray-800 dark:text-gray-100">Cash Flow {year}</h2>
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-3 text-xs mr-2">
              ...
              <span className="flex items-center gap-1.5 hidden sm:flex text-gray-600 dark:text-gray-300"><span className="w-3 h-1.5 rounded-full bg-indigo-500" /> Akumulasi</span>
            </div>
            <div className="flex items-center bg-gray-100 dark:bg-gray-700 rounded-lg overflow-hidden">
              <button onClick={() => setYear(year - 1)} className="px-2 py-1.5 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-400 dark:text-gray-500 transition-colors"><ChevronLeft size={14} /></button>
              <span className="px-2 text-xs font-semibold text-gray-600 dark:text-gray-300">{year}</span>
              <button onClick={() => setYear(year + 1)} className="px-2 py-1.5 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-400 dark:text-gray-500 transition-colors"><ChevronRight size={14} /></button>
            </div>
          </div>
        </div>
        {cashFlowData.some(d => d.Pemasukan > 0 || d.Pengeluaran > 0) ? (
          ...
          <div className="flex items-center justify-center h-[220px] text-gray-400 dark:text-gray-500 text-sm">Belum ada data transaksi tahun {year}</div>
        )}
      </div>

      {/* Goal Cards */}
      {goals.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 transition-colors">
          <PiggyBank size={48} className="mx-auto text-gray-300 dark:text-gray-600 mb-4" />
          <p className="text-gray-500 dark:text-gray-400 font-medium text-sm">Belum ada target tabungan</p>
          <p className="text-gray-400 dark:text-gray-500 text-xs mt-1">Buat target pertamamu dan mulai menabung!</p>
        </div>
      ) : (
        <div className="space-y-3 sm:grid sm:grid-cols-2 sm:gap-4 sm:space-y-0">
          {goals.map((goal) => {
            ...
            return (
              <div key={goal.id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm overflow-hidden group transition-colors">
                ...
                <div className="p-4 sm:p-5">
                  ...
                  {!goal.image_url && (
                    <div className="flex items-start justify-between mb-3 sm:mb-4">
                      <div className="flex-1 min-w-0">
                        <h3 className="font-semibold text-gray-800 dark:text-gray-100 text-base sm:text-lg truncate">{goal.name}</h3>
                        {goal.deadline && (
                          <div className="flex items-center gap-1 text-[10px] sm:text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                            <Calendar size={12} />
                            {daysLeft !== null && daysLeft > 0 ? `${daysLeft} hari lagi` : daysLeft === 0 ? 'Hari ini!' : `Terlewat ${Math.abs(daysLeft)} hari`}
                          </div>
                        )}
                      ...
                      <div className="flex items-center gap-0.5 sm:gap-1 ml-2">
                        <button onClick={() => openFund(goal, 'deposit')} className="p-1.5 sm:p-2 text-green-500 hover:bg-green-50 dark:hover:bg-green-900/30 rounded-lg transition-colors"><ArrowUpCircle size={18} /></button>
                        <button onClick={() => openFund(goal, 'withdraw')} className="p-1.5 sm:p-2 text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/30 rounded-lg transition-colors"><ArrowDownCircle size={18} /></button>
                        <button onClick={() => openEdit(goal)} className="p-1 sm:p-1.5 text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg transition-colors hidden sm:block"><Edit2 size={15} /></button>
                        <button onClick={() => handleDelete(goal.id)} className="p-1 sm:p-1.5 text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors hidden sm:block"><Trash2 size={15} /></button>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="mb-2.5 sm:mb-3">
                    <div className="flex items-end justify-between mb-1.5 sm:mb-2">
                      ...
                      <span className={`text-xs sm:text-sm font-semibold px-1.5 sm:px-2 py-0.5 rounded-full ${isComplete ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300'}`}>
                        {pct}%
                      </span>
                    </div>
                  ...
                  {!isComplete ? (
                    <p className="text-[10px] sm:text-xs text-gray-400 dark:text-gray-500">
                      Sisa <span className="font-medium text-gray-600 dark:text-gray-300">Rp <span className="sm:hidden">{formatCompact(remaining)}</span><span className="hidden sm:inline">{formatRupiah(remaining)}</span></span> lagi
                    </p>
                  ) : (
                    <p className="text-[10px] sm:text-xs text-green-600 dark:text-green-400 font-medium">Target tercapai!</p>
                  )}
                </div>
              </div>
            );
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Target' : 'Target Tabungan Baru'}>
        <form onSubmit={handleSave} className="space-y-4">
          ...
          {/* Image upload */}
          <div>
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Foto Target (opsional)</label>
            {imageURL ? (
              ...
              >
                <ImagePlus size={28} />
                <span className="text-sm text-gray-500 dark:text-gray-400">Tambah foto (misal: foto barang yang mau dibeli)</span>
                <span className="text-xs text-gray-300 dark:text-gray-600">JPG, PNG - maks ~500KB setelah kompresi</span>
              </button>
            )}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nama Target</label>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Contoh: iPhone 16, Liburan ke Jepang..." className="w-full border border-gray-200 dark:border-gray-600 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm bg-white dark:bg-gray-700 dark:text-gray-100" autoFocus />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Jumlah Target (Rp)</label>
            <input type="number" value={targetAmount} onChange={(e) => setTargetAmount(e.target.value)} placeholder="10000000" min="1" className="w-full border border-gray-200 dark:border-gray-600 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm font-semibold bg-white dark:bg-gray-700 dark:text-gray-100" inputMode="numeric" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Deadline (opsional)</label>
            <input type="date" value={deadline} onChange={(e) => setDeadline(e.target.value)} className="w-full border border-gray-200 dark:border-gray-600 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm bg-white dark:bg-gray-700 dark:text-gray-200" />
          </div>
          <div>
      ...
      <Modal open={fundModalOpen} onClose={() => setFundModalOpen(false)} title={fundMode === 'deposit' ? `Tambah ke "${fundGoal?.name}"` : `Tarik dari "${fundGoal?.name}"`}>
        <form onSubmit={handleFund} className="space-y-4">
          ...
          {fundGoal && (
            <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-3.5 sm:p-4">
              <div className="flex justify-between text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-1">
                <span>Terkumpul</span>
                <span className="font-medium text-gray-700 dark:text-gray-200">Rp {formatRupiah(fundGoal.current_amount)}</span>
              </div>
              <div className="flex justify-between text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                <span>Target</span>
                <span className="font-medium text-gray-700 dark:text-gray-200">Rp {formatRupiah(fundGoal.target_amount)}</span>
              </div>
            </div>
          )}
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{fundMode === 'deposit' ? 'Jumlah Ditambahkan' : 'Jumlah Ditarik'} (Rp)</label>
            <input type="number" value={fundAmount} onChange={(e) => setFundAmount(e.target.value)} placeholder="500000" min="1" className="w-full border border-gray-200 dark:border-gray-600 rounded-xl px-4 py-3 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none text-sm font-semibold bg-white dark:bg-gray-700 dark:text-gray-100" autoFocus inputMode="numeric" />
            <div className="grid grid-cols-4 gap-2 mt-2.5">
              {[100000, 500000, 1000000, 5000000].map((amt) => (
                <button key={amt} type="button" onClick={() => setFundAmount(amt.toString())} className="py-2 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-xl text-xs font-medium text-gray-600 dark:text-gray-300 transition-colors">
          ...
          {fundGoal && fundAmount && (
            <div className="bg-blue-50 dark:bg-blue-900/30 rounded-xl p-3.5 text-sm">
              <p className="text-blue-700 dark:text-blue-300">
                ...
                <span className="text-blue-400 dark:text-blue-500"> / Rp {formatRupiah(fundGoal.target_amount)}</span>
              </p>
            </div>
          )}
